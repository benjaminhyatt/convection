...
dwQ = 5e-2 #2e-2 # scale of Q jump
L_CZ = 1e0 # where to center jump in flux
dwk = 1e-1 #7.5e-2 # scale of k jump
dwT = 1e-1 #5e-2 # scale of T0_zz jump
...
stop_sim_time = 3e4 # freefall times
output_suffix = 'mu_{:.0e}'.format(mu) + 'R_{:.0e}'.format(R)
...
fields_path = setup_dir + '/setup_fields.h5'
...
if not restart: 
    ...
    write, init_dt = solver.load_state(start_dir)
    print("Read in solver.state from checkpoint, setting init_dt to half of", init_dt)
    init_dt *= 0.5
...
profiles_0 = solver.evaluator.add_file_handler(profiles_0_dir, sim_dt = np.inf, mode=file_handler_mode) # will output at t=0
